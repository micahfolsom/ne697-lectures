...
// For "primitive" types (int, bool, char, float, etc), use this instead of
// by-const-reference (they're generally smaller than the address would be!)
void by_value(int x) {
  // No effect outside this function! x is a local variable
  x += 1000;
  return;
}
...
// For primitive types (like int), you should pass-by-value, since the size
// of the value (4 bytes on my system) is smaller than the address (8 bytes
// on my system).
// If the type if x is large, though, this could save us quite a bit
